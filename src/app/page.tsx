...
function Logo() {
  return (
    <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 8, textDecoration: 'none' }}>
      ...
      <span style={{ fontFamily: "'Instrument Serif', serif", fontSize: 20, color: 'var(--ink)' }}>SitemapAI</span>
    </a>
  );
}
...
function Features() {
  const items = [{ icon: '🔍', title: 'Instant Sitemap Discovery', desc: 'Find your sitemap in seconds.' },{ icon: '🤖', title: 'AI-Powered Analysis', desc: 'Specific recommendations, not generic advice.' },{ icon: '📊', title: 'Prioritized Fix List', desc: 'Ranked list of what to fix and create.' },{ icon: '⚡', title: 'Under 60 Seconds', desc: 'Full AI report, free, no signup.' },{ icon: '🗺️', title: 'All Formats', desc: 'WordPress, Shopify, Next.js all work.' },{ icon: '🎯', title: 'Site-Specific', desc: 'Based on your actual URL patterns.' }];
  return (<section style={{ maxWidth: 960, margin: '0 auto', padding: '80px 24px' }}><div style={{ textAlign: 'center', marginBottom: 56 }}><h2 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 36, color: 'var(--ink)', marginBottom: 14 }}>Why SitemapAI?</h2><p style={{ fontSize: 16, color: 'var(--muted)', maxWidth: 480, margin: '0 auto' }}>Most SEO tools give scores. We give a specific action plan.</p></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>{items.map(item=>(<div key={item.title} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 14, padding: '28px 24px' }}><div style={{ fontSize: 28, marginBottom: 14 }}>{item.icon}</div><div style={{ fontSize: 15, fontWeight: 700, color: 'var(--ink)', marginBottom: 8 }}>{item.title}</div><div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.65 }}>{item.desc}</div></div>))}</div></section>);
}

function HowItWorks() {
  const steps = [{ num: '1', title: 'Enter your domain', desc: 'Type any domain.' },{ num: '2', title: 'We find your sitemap', desc: 'Fetch every URL and cluster them.' },{ num: '3', title: 'AI generates your plan', desc: 'Specific, ranked recommendations.' },{ num: '4', title: 'Fix and grow', desc: 'Create missing pages, build links.' }];
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper)' }}>
      ...
      <main>
        ...
        {state === 'error' && (<div style={{ maxWidth: 540, margin: '80px auto', textAlign: 'center' }}><h2 style={{ fontSize: 24, fontWeight: 700, color: 'var(--ink)', marginBottom: 12 }}>Sitemap not found</h2><p style={{ fontSize: 15, color: 'var(--muted)', marginBottom: 32 }}>{error}</p><button onClick={reset} className="cta-btn">Try another domain</button></div>)}
