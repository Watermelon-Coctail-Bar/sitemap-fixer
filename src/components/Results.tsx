    ...
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 20px 80px' }}>
      ...
      {clusters.length > 0 && (
        <div className="card anim-fade-up anim-fade-up-2" style={{ marginBottom: 24 }}>
          ...
          {/* When filter active, always show affected URLs from the matching issues */}
          {filter && (() => {
            const matchingIssues = issues.filter(iss => iss.severity === filter);
            if (matchingIssues.length === 0) {
              return (
                <div style={{ padding: '20px 16px', textAlign: 'center' }}>
                  <div style={{ fontSize: 13, color: 'var(--muted)' }}>No {SEV[filter]?.label?.toLowerCase() || filter} issues found in analyzed URLs.</div>
                </div>
              );
            }
            return (
              <div style={{ padding: '16px' }}>
                {matchingIssues.map((iss, ii) => (
                  <div key={ii} style={{ marginBottom: ii < matchingIssues.length - 1 ? 16 : 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <span style={{ width: 7, height: 7, borderRadius: '50%', background: SEV[iss.severity]?.dot || '#6b7280', flexShrink: 0 }} />
                      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ink)' }}>{iss.problem}</span>
                    </div>
                    {iss.fix && <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 8, paddingLeft: 15 }}>Fix: {iss.fix}</div>}
                    {(iss.affectedUrls || []).length > 0 ? (
                      ...
                      <div style={{ fontSize: 12, color: 'var(--muted)', paddingLeft: 15 }}>No specific URLs identified — applies to overall sitemap structure.</div>
                    )}
                  </div>
                ))}
              </div>
